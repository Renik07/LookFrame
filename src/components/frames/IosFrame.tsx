...
export default function IosFrame({ url, device, baseWidth, baseHeight, scale }: FrameProps) {
			...
			{panelMode !== "off" && (
				...
				>
					...
					<div className={`bg-[#8e8e931a] flex items-center justify-between ${(panelMode === "float" && isHovered) ? "h-[100%] p-0 rounded-none" : "h-[45%] p-2 rounded-xl"}`}>
						...
						<div className="text-sm text-black transition-all duration-300 ml-auto mr-auto">kg-portal.ru</div>
						{(panelMode === "fixed" || (panelMode === "float" && !isHovered)) && (
